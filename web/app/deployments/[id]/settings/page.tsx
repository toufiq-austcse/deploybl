...
const SettingsPage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    validateBuildAndDeployForm.setValue(
      'root_directory',
      deploymentDetails?.root_directory == null ? '' : deploymentDetails?.root_directory,
    );
  ...
  const onGeneralUpdateFormSubmit = async (values: z.infer<typeof generalUpdateSchema>) => {
    ...
    updateDeploymentDetails(deploymentDetails?._id, { title: values.title })
      .then((response) => {
        if (response.error) {
      ...
      <div className="border 1px solid black p-10 space-y-3">
        ...
        <Form {...validateGeneralUpdateForm}>
          <form id="repo-url-form" onSubmit={validateGeneralUpdateForm.handleSubmit(onGeneralUpdateFormSubmit)}>
            <div className="gap-4">
              <FormField
                control={validateGeneralUpdateForm.control}
                name="title"
                render={({ field }) => (
                  <FormItem>
                    <div className="flex flex-col space-y-2">
      ...
      <div className="border 1px solid black p-10 space-y-3">
        ...
        <Form {...validateBuildAndDeployForm}>
          ...
          >
            <div className="my-2">
              <FormField
                control={validateBuildAndDeployForm.control}
                name="branch_name"
                render={({ field }) => (
                  <FormItem>
                    <div className="flex flex-col space-y-2">
