...
const VideoTitleComponent = ({ videoDetails }: { videoDetails: VideoDetails }) => {
  ...
  return enableEditTitle ? (
    <div className="flex gap-4">
      ...
      ></Input>
      <Button variant="outline" size="icon" className="bg-blue-500 text-white" onClick={onUpdateClick}>
        <Check className="h-4 w-4" />
      </Button>
      <Button variant="outline" size="icon" className="bg-red-500 text-white" onClick={onCancelClick}>
        <X className="h-4 w-4" />
      </Button>
    </div>
  ) : (
